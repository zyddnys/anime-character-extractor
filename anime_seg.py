...
def get_seg_image(img: np.ndarray) -> np.ndarray :
    global ANISEGNET
    if ANISEGNET is None :
        ANISEGNET = AnimeSegmentation.try_load('isnet_is', 'isnetis.ckpt', 'cuda:0')
    mask = get_mask(ANISEGNET, img, use_amp=True, s=768)
    valid_mask_region = (mask > 0.3).astype(np.uint8) * 255
    x,y,w,h = cv2.boundingRect(valid_mask_region)
    white_bg = np.ones_like(img) * 255
    img_out = np.clip(mask * img.astype(np.float32) + (1 - mask) * white_bg.astype(np.float32), 0, 255).astype(np.uint8)
    return img_out[y:y+h,x:x+w], img[y:y+h,x:x+w], (mask[y:y+h,x:x+w] * 255).astype(np.uint8)
